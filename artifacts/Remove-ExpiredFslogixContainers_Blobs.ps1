...
try
{
	...
	try 
	{
		Write-Log -StorageAccountName $StorageAccountName -Message "List blobs for '$StorageAccountName/$StorageContainerName'"
		$Uri = 	'https://' + $StorageAccountName + '.blob.' + $StorageEndpoint + '/' + $StorageContainerName + '?restype=container&comp=list'
		$BlobsList = Invoke-RestMethod -Headers $Header -Method 'Get' -Uri $Uri
	...
	{
		throw [System.Exception]::new("Failed to list blobs for '$StorageAccountName/$StorageContainerName'. Ensure that you have entered the correct values", $PSItem.Exception)
	}
